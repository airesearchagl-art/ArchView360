...
| ④ 関数レベルガードEditor専用（`project-info-modal`系9・`set-name-modal`系8・`group-picker`系4） | 21 | ID基準 | 確認済み値（暫定、13節の未確認事項が未解消のため増加しうる） | 除去可能性が高いが、13節が未解消のため`[未検証]`のまま残る（5節参照） |
| 確認済みEditor専用合計（①+②+④、ID基準） | **48**（暫定） | ID基準 | 確認済み値（暫定＝27は確定だが21が暫定のため合計も暫定） | — |
| ⑤ Common候補（184－48－0、ID基準） | **136**（暫定） | ID基準 | 候補値（暫定、48の暫定性と連動） | 除去不可（Viewer専用ページにも必須） |
| ⑥ Viewer専用（`.viewer-only`実使用） | 0 | ID基準 | 確定値 | 該当なし |

- **ID基準でのViewer専用ページの除去候補・残存Common数**: ①＋②＋④＝27＋21＝
  **48（暫定）**。184－48－0＝**136（暫定）**がID基準のCommon残存数になる。
  これはDOM調査文書2.7節の「確認済みEditor専用合計48（ID基準）」「Common候補136
  （候補値・暫定）」と完全に同一の集合・同一の暫定性を継承する`[事実に基づく整理]`。
  **①②（27件）はこの48件の内訳のうち確定している部分であり、④（21件）が
  13節の解消によって増減すれば、48・136もそれに応じて増減する**
- ③（id無しコンテナ`floormap-info-actions`本体、1件）は184のID基準集計には
  ...
  混在させないため）
- **本文書では、単位の異なる値を合算した「49」「135」という数値は使用しない**。
  Viewer専用ページの除去候補・残存Common数は、確定するまでは常にID基準の
  「48（暫定）」「136（暫定）」を用いる。①②合計の27（HTMLノード基準では
  ③を加えた28）はいずれも確定値だが、48・136自体は④（21、暫定）を内数に含むため
  **13節の未確認事項が解消するまで確定値にならない**
...
|---|---|---|---|---|
| DOM削減効果 | 無し（184件常時全部） | 高い（viewer.htmlは2節のID基準48件・暫定を除いたCommon136件・暫定相当。加えてid無しコンテナ1件も併せて除去） | 案2と同等（viewer.html側）。editor.html側はViewer専用要素が現状0件のため追加削減はほぼ無い | 案2/3と同じ削減結果を、生成元テンプレートから機械的に達成 |
...
- `viewer.html`は184件（ID基準）中、2節の①＋②＋④＝27＋21＝**48件（暫定、
  13節解消待ち）**を除いた**136件（暫定）のCommon要素**を中心に含む。加えて、
...
- テストは2段階に分けて追加する（6節のPR分割案と対応させる）`[提案]`:
  - **最低限の起動・主要表示テスト**（`viewer.html`追加PRに同梱、6節PR-B）:
    1. `viewer.html`で`init()`が例外なく完走すること（DOM基準48件＋id無し
...
2. `viewer.html`を新規追加する。`index.html`から2節のID基準48件（暫定）＋
...
1. **PR-A**: DOM調査文書13節の未確認事項の解消（他モーダルの関数レベル
   ガードの網羅調査、docs-only）。2節の④（21件、暫定）の除去可否を確定させる
   前提作業として先行させる
2. **PR-B**: `viewer.html`静的ファイルの新規追加（2節のID基準48件・暫定＋
...
- `docs/ViewerEditor_DOM_Responsibility_Investigation.md`（Phase 2調査。2節の
  ID基準48件・HTMLノード基準28件・Common候補136件の出典、13節の未確認事項）
