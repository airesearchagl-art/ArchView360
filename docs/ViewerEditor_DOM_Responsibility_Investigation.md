...
（パターンBの子要素）を加えた計27〜28要素になる（CSSベース分に限った
確定値・集計規則は2.7節参照。ただしEditor専用の**全体**件数は関数
レベルガード分も含むため暫定値である点に注意、2.4節・2.7節参照）。
将来、`.editor-only`要素をHTMLから機械的に除去する場合、
**クラスを持つ要素だけを見ているとパターンBの子要素8件を見落とす**
`[事実に基づく評価]`。

### 2.7 件数の単位定義と集計表（27件はCSSベースの確定値、48件・136件は現時点の確認済み値・暫定分類）
...
**Editor専用の集計表**（表内の「確定値」欄と「確認済み値・暫定」欄を区別する）

| 区分 | 件数 | 確度 | 単位 | 内訳 |
|---|---|---|---|---|
| 直接`.editor-only`付与（id付き） | 19 | 確定値 | ユニークID | 操作系ボタン18＋コンテナ`floormap-orient-bar`1 |
| 直接`.editor-only`付与（id無し） | 1 | 確定値 | HTMLノード（184の外） | コンテナ`floormap-info-actions`（script.js参照0件） |
| 直接付与 合計（HTMLノード基準） | 20 | 確定値 | HTMLノード | 19＋1。`grep -c "editor-only" index.html`の20と一致 |
| 親`.editor-only`からの継承 | 8 | 確定値 | ユニークID | `floormap-orient-bar`の子4（`floormap-orient-l`/`-val`/`-r`/`-preset`）＋`floormap-info-actions`の子4（`floormap-rename-btn`/`floormap-rot-l`/`-r`/`floormap-del-mk`）。8件はすべて`id`を持つ |
| **CSSベースEditor専用（ID基準）** | **27** | **確定値**（`.editor-only`の全出現箇所を`grep`で網羅したため） | ユニークID | 19（直接、id付き）＋8（継承、id付き）。**本文書ではこの27を正式値として採用する** |
| CSSベースEditor専用（HTMLノード基準、参考値） | 28 | 確定値 | HTMLノード | 27＋`floormap-info-actions`（id無し）1件。27との差1件の原因はこの1要素のみ |
| 関数レベルガード（CSSクラス無し） | 21 | **確認済み値（暫定）**——本調査で個別確認した3モーダル群のみの件数。同様のパターンを持つ他のUIが未確認のまま残っており（13節）、見つかれば増加する | ユニークID | `project-info-modal`系9＋`set-name-modal`系8＋`group-picker`系4（4.2節） |
| CSS分類との重複 | 0 | 確定値（確認した範囲内で） | — | 上記21件がCSSベース27件のいずれとも重ならないことを個別確認済み（`grep`で各idの`class`属性に`editor-only`が無いことを確認） |
| **確認済みEditor専用 合計（ID基準）** | **48** | **確認済み値（暫定）**——27（確定）＋21（暫定）の合算のため全体としては暫定 | ユニークID | 27（CSSベース）＋21（関数レベル）－0（重複）。未確認の関数ガードUIが見つかれば増加する |
| Viewer専用 | 0 | 確定値 | ユニークID | `.viewer-only`実使用0件（既存事実） |
| **Common候補（184－48－0）** | **136** | **候補値（暫定）** | ユニークID | 4.3節参照。未確認の関数ガードUIが見つかれば減少する |

**184件との合計関係の検算（現時点の暫定分類）**: 48（確認済みEditor専用）
＋0（Viewer専用）＋136（Common候補）＝184。この検算は**現時点で確認できた
範囲での分類**であり、184という母数自体は確定だが、48/136という内訳は
確定値ではない。**未確認の関数レベルガードUIが新たに見つかった場合、
Editor専用は48件から増加し、Common候補は136件から減少する**（合計は
常に184で一致し続ける）。状態依存は独立した横断属性のため、この合計には
加算しない（3節・4.4節参照）`[事実＋提案]`。
...
- **Editor専用**: 以下のいずれかにより、Viewerでは到達・表示されない要素
  - `.editor-only`クラス（直接付与、または親コンテナからの継承）
  - 要素を開く関数自体に`assertEditorMode()`等のmode guardがある
    （例: `project-info-modal`、`group-picker`）
  - 要素を開く関数自体にはguardが無いが、**調査時点で確認できた全ての
    到達可能な呼び出し元**それぞれに`assertEditorMode()`等のmode guardが
    ある（例: `set-name-modal`。`openSetNameModal()`自体は無guardだが、
    呼び出し元3箇所すべてを確認した結果としてEditor専用と分類した、
    4.2節参照）。この3つ目の基準は「呼び出し元を網羅的に確認できた
    範囲でのみ」成立する分類であり、未確認の呼び出し元が後から
    見つかれば再分類が必要になる`[提案]`
...
3つは**構造上は互いに排他な分類**であり、現時点で確認できた範囲では
2.7節の集計表のとおり`48（確認済みEditor専用）＋0（Viewer専用）
＋136（Common候補）＝184`と過不足なく合計できる。ただし**48と136の
内訳自体は暫定分類であり、確定値ではない**（2.7節参照。関数レベル
ガードの網羅確認が未完了のため、13節）。一方、**「状態依存」はこの
3分類と排他ではなく、いずれの分類にも横断的に付与されうる属性
（タグ）である**。例えば`project-info-modal`は「Editor専用」**かつ**
「状態依存」（開いているかどうかで表示が変わる）であり、
`dirty-indicator`や`import-modal`は「共通」**かつ**「状態依存」である。
**「状態依存」の要素数を184件の内訳として別途加算することはしない**
（Editor専用（暫定48）・Viewer専用0・Common候補（暫定136）のいずれかに
属する要素の一部が、追加で状態依存というタグも持つ、という関係）
`[提案]`。
...
184要素中、4.1（27件、確定値）・4.2（21件、現時点の確認済み値）を除いた
残り**136要素（2.7節の確認済み値・暫定、13節の未確認事項が解消されれば
減少しうる）**は原則Common候補だが、本調査では以下を重点的に個別確認した。
...
| ⑤ 縮小HTMLで除去予定（guard状況によらず将来のViewer専用HTMLからは除外） | 48（確認済みEditor専用全体、2.7節、暫定） | 27（CSSベース、確定）＋21（関数ガード系、確認済み・暫定）。①〜④のタグとは独立に、全Editor専用要素に付与される「将来の除去計画」タグ。48自体が暫定のため、この件数も未確認調査の結果次第で増加しうる |
| ⑥ 現行では変更不要 | 142（暫定） | 6件（CSSベースguard済み、確定）＋136件（Common候補、4.3節、暫定）。Viewer専用0件は該当無し。136が暫定のため142も暫定 |

**①＋②＝3＋18＝21（6.1節の「無guard21件」と一致）**`[事実]`。この21件
（CSSベース27件の内数）は確定値である。

### 6.3 「無guard21件」の位置づけ（現行不具合ではない）`[提案]`

**重要な前提**: 現行の`index.html`は単一HTMLであり、6.2節①②の21件を
含む184件すべてのDOM要素が**常に存在している**。したがって、これらが
無guardであること自体は、**現行の単一HTML構成において何らかの不具合を
引き起こしているわけではない**。無guardが問題になるのは、あくまで
**将来Phase 3で実際に縮小版HTML（`viewer.html`相当）を作り、これらの
要素を物理的に除去した場合**に限られる。

- 6.2節①②の21件は、**縮小HTMLで該当DOMを除去する場合の安全化検討候補**
  であり、現行の単一HTML構成における即時の不具合21件ではない`[提案]`
- 具体的にどの候補にどこまで対応するか（12節の候補A〜E）は、Phase 3の
  設計（縮小HTMLを実際に作るかどうか、作る場合の除去範囲）が固まった
  段階で個々に決定する。本調査では候補の洗い出しと分類に留める`[提案]`

### 6.4 確定値と要追加調査の区別

- **確定値（本調査で個別に全件確認済み）**: 6.1節・6.2節の①②＝21件
  （CSSベース27件の内数）は、すべて`grep`による個別確認を完了しており、
  `[未検証]`は残っていない`[事実]`
- **確認済み値（暫定、要追加調査）**: 4.2節の「関数ガード系21件」と
  同様のパターンを持つ他のUIが存在するかどうかは、本調査では3例のみ
  確認しており、網羅確認は行っていない（13節参照）。この網羅確認が
  完了した場合、③④の集合（現在21件）・2.7節の48/136（Editor専用/
  Common候補）が変動する可能性があるが、①②（CSSベースの無guard21件、
  確定値）の数には影響しない`[未検証]`
...
1. 6.2節①②で確定した**CSS無guard系21件**（DOM取得/呼び出しguard必要3件
   ＋イベント登録guard必要18件）を、縮小HTMLで該当DOMを実際に除去する
   場合の安全化検討候補として扱う（6.3節参照。現行の単一HTML構成では
   不具合ではないため、一律の即時対応ではなく、このリストに基づく
   個別判断とする）。**4.2節の関数ガード系21件は、開く関数のガードに
   より既に安全なため、この対応対象には自動的に含めない**（同じ「21」
   という数字だが別集合、2.4節・6節参照）
...
3. **13節の未確認事項（他の関数レベルガード付きUIの網羅確認）を解消する。
   この確認が完了するまでは、2.7節のEditor専用48件・Common候補136件は
   暫定分類のままであり、Phase 3の対象範囲を最終確定する前提として、
   本項目の完了を先行させる**
...
- 4.2節で確認した3つの関数レベルガードモーダル以外に、同様のパターン
  ...
  持つUIが他に存在するか（6.4節参照）。**この調査が完了すると、2.7節の
  Editor専用48件は増加し、Common候補136件は減少する**。一方、6.2節①②の
  「CSSベースの無guard21件（確定値）」自体はこの調査の結果に影響されない
