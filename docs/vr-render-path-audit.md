...
y座標は当初`0`で実装しましたが、WebXRの`local-floor`基準では床がy=0になるため、
装着者の目線（立位でおよそ1.5〜1.7m）よりかなり下、ほぼ床面付近にCubeが表示されて
しまい、「描画には成功しているが発見しにくい」誤判定を招く恐れがあると指摘があり、
マージ前に`y=1.6`（目線付近）へ修正しました。
